...
local function resolveYouTubeToAudioUrl(url, callback)
	...
	local function tryEndpoints(endpointList, onDone)
		local index = 1
		local function tryNextEndpoint()
			if index > #endpointList then
				onDone(nil)
				return
			end

			local endpoint = endpointList[index]
			index = index + 1
	...
	tryEndpoints(endpoints, function(primaryResolved)
		if primaryResolved then
			callback(primaryResolved)
			return
		end

		PerformHttpRequest("https://api.invidious.io/instances.json?sort_by=health", function(statusCode, body)
			if statusCode < 200 or statusCode >= 300 or not body then
				callback(nil)
				return
			end
			...
			for _, item in pairs(payload) do
				if type(item) == "table" then
					local host = item[1]
					local meta = item[2]
					if type(host) == "string" and type(meta) == "table" and meta.api and (meta.type == "https" or meta.type == nil) then
						local endpoint = ("https://%s"):format(host)
						if not seen[endpoint] then
							seen[endpoint] = true
							table.insert(discovered, endpoint)
						end
					end
				end
			end
			...
			local maxDynamic = math.min(#discovered, Config.YouTubeDynamicResolverLimit or 12)
			local shortlist = {}
			for i = 1, maxDynamic do
				shortlist[#shortlist + 1] = discovered[i]
			end

			tryEndpoints(shortlist, function(dynamicResolved)
				callback(dynamicResolved)
			end)
		end, "GET", "", { ["Accept"] = "application/json" })
	end)
end
