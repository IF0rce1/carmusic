...
function engineTick() {
  Object.keys(carplayEngine.sounds).forEach((name) => {
    ...
    if (!data || (!data.howl && !data.fallbackAudio)) return
    ...
    if (Math.abs((data.lastAppliedVolume || 0) - safeVol) > 0.008) {
      if (data.howl) data.howl.volume(safeVol)
      if (data.fallbackAudio) data.fallbackAudio.volume = safeVol
      data.lastAppliedVolume = safeVol
    }
    if (data.howl && typeof data.howl.stereo === 'function') {
...
function createHtmlAudioFallback(entry, url, loop) {
  try {
    ...
    audio.preload = 'auto'
    audio.volume = 0
    audio.play().then(() => showStatus('Fallback audio conectat.')).catch(() => {})
    entry.howl = null
    entry.fallbackAudio = audio
    return audio
  } catch (_) {
    return null
  }
  ...
  const howl = new Howl({
    ...
    onloaderror: () => {
      if (retries < 3) {
        ...
        const fallback = createHtmlAudioFallback(entry, url, loop)
        if (!fallback) {
          showStatus('Nu s-a putut încărca stream-ul (URL/CORS).', true)
        } else {
          showStatus('Stream pe fallback mode (compatibilitate).')
        }
      }
    },
    onplayerror: () => {
...
window.addEventListener('message', function (event) {
  ...
  if (data.cmd === 'destroy') {
    ...
    if (existing && existing.fallbackAudio) {
      existing.fallbackAudio.pause()
      existing.fallbackAudio.src = ''
      existing.fallbackAudio.load()
    }
  ...
  if (data.cmd === 'upsert') {
    ...
    if (entry.howl) {
      ...
      entry.fallbackAudio.loop = entry.loop
      if (data.paused) entry.fallbackAudio.pause()
      else entry.fallbackAudio.play().catch(() => {})
    }
    applyFx(entry)
    return
  }
  ...
  if (!snd || (!snd.howl && !snd.fallbackAudio)) return
  if (data.cmd === 'pause') {
    if (snd.howl) snd.howl.pause()
    if (snd.fallbackAudio) snd.fallbackAudio.pause()
  }
  if (data.cmd === 'resume') {
    if (snd.howl && !snd.howl.playing()) snd.howl.play()
    if (snd.fallbackAudio) snd.fallbackAudio.play().catch(() => {})
  }
  if (data.cmd === 'setUrl') {
    if (snd.howl) snd.howl.unload()
    if (snd.fallbackAudio) {
      snd.fallbackAudio.pause()
      snd.fallbackAudio.src = ''
      snd.fallbackAudio.load()
      snd.fallbackAudio = null
    }
  ...
  if (data.cmd === 'setLoop') {
    snd.loop = !!data.value
    if (snd.howl) snd.howl.loop(snd.loop)
    if (snd.fallbackAudio) snd.fallbackAudio.loop = snd.loop
  }
  if (data.cmd === 'setFx') {
    snd.fxPreset = data.value || {}
    applyFx(snd)
  }
  if (data.cmd === 'seek' && snd.howl && typeof snd.howl.seek === 'function') snd.howl.seek(data.value || 0)
})
